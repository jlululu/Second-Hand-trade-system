...
namespace SecondHand.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            WebSecurity.Logout();
            Session.Remove("usersame");                 //加的
            Session.Remove("password");                 //加的
                ...
                try
                {
                    if (ModelState.IsValid)
                    {
                        ...
                        using (UsersContext dbtest = new UsersContext())
                        {
                            ...
                            UserProfile setuser = dbtest.UserProfiles.Single(i => i.UserName.Equals(a));
                            setuser.Email = Request["email"];
                            setuser.Phonenumber = Request["number"];
                            setuser.Registerdate = DateTime.Now.ToString();
                            dbtest.SaveChanges();  
                        }
                        return RedirectToAction("Index", "Home");
                    }
                    else { 
                        ViewData["re"] = "注册失败，身份验证输入错误,请重新注册";
                    }
                }
